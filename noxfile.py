...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/presentation/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/presentation/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/presentation/*"
            )
            ...
            #    "--include=src/saltext/presentation/*,tests/*"
            # )
        finally:
